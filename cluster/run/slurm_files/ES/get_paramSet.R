...
param_limits <- data.frame(un_year = c(1972, 1978),         # Year inflexion point diffusion of contraception.
                           un_max = c(0.35, 0.75),          # Maximum Risk Unplanned births
                           un_min = c(0.025, 0.040),        # minimum risk of unplanned birth
                           uk = c(0.15, 0.27),              # Speed of diffusion contraception
                           th_edu = c(14, 24),              # Threshold years of education
                           w_pnty = c(0.4, 0.7),            # Max effect work
                           post_edu = c(5, 8),              # years after end of education for family formation
                           gamma = c(35, 45),               # Fecundability age
                           kappa = c(0.1, 0.7),             # Fecundability rate
                           d = c(0.05, 0.09),               # reduction risk unplanned after achieve D
                           ini_d = c(2.7, 3),             # initial value desired family size
                           diff_d = c(0.15, 0.3),          # delta D
                           d_norm = c(1.6, 2)               # inflection point resistance of D
)
