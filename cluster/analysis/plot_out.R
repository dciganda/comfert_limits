plot_out <- function(sim, country, params, iniY) {
  ...
  # 
  # # PLOT
  # plot(mean_asfr[1,], type="l", col=redCol[1], lwd=4, cex.lab=1.6, cex.axis=1.6,
  #      ylab="Age Specific Fertility Rates", xlab="Age", xaxt='n', frame.plot=F,
  #      ylim = c(0,0.2),
  #      main = paste("Simulated ASFR", country))
  ...
  if(country != "FR"){
  obs_ta_tfr <- read.table(paste0("../run/data/",country,"/out/ta_tfr_hfd.txt"), skip = 2, header = T,
                         stringsAsFactors = F)
  }
  ...
  plot(seq(y1,max(obsTfr$Year),1),c(obsTfr[obsTfr >= y1,2]),
       ...
       cex.main=1, cex.axis=1.6, cex.lab=1.6 ,type="l", pch=20, 
       ylab="TFR", xaxt='n', frame.plot=F,
       xlim = c(y1, endY))
  lines(seq(y1,endY-1,1), mean_tfr[-c(1:c(y1-iniY), 108:length(mean_tfr))], cex=1, pch=20, col = "red", lwd = 4)
  for (i in 1:length(tfr)){
    lines(seq(y1,endY-1,1), tfr[[i]][-c(1:c(y1-iniY), 108:length(mean_tfr))], lty = 3, col = "red")
  }
  if(country != "FR"){
    lines(obs_ta_tfr$Year, obs_ta_tfr[,2], lty = 2, lwd =3)
  }
  axis(side=1, at=seq(y1, y2+10, 5), label= seq(y1, y2 + 10, 5), cex.axis=1.6)
  legend("topright", c("Observed","Tempo Adjusted", "Simulated"), pch=20, pt.cex = 2, bty="n", cex=1.6,
  ...
  # Obs
    obs_ccf <- read.table(paste0("../run/data/",country,"/out/ccf_hfd.txt"), skip = 2, header = T,
                         stringsAsFactors = F)
  ...
  plot(obs_ccf[obs_ccf[,1] %in% iniY:(endY -51),1], obs_ccf[obs_ccf[,1] %in% iniY:(endY -51),2],
       ...
       cex.main=1,cex.axis=1.6, cex.lab=1.6, type="l", cex=1, pch=20,
       ylab="Cohort Completed Fertility", xaxt='n',
       frame.plot = F)
  lines(mean_ccf$year,mean_ccf$val, cex=1, pch=20, col = "red", lwd = 4) 
  for (i in 1:length(ccf)){
    lines(seq(iniY,(endY -31),1), ccf[[i]]$val, lty = 3, col = "red")
  }
  axis(side = 1, at = seq(iniY, y2+10, 5), label= seq(iniY, y2 + 10, 5), cex.axis=1.6)
  legend("topright", c("Observed", "Simulated"),pch=20,
         pt.cex = 2, bty="n", cex=1.6, col = c("black","red"))
  
  
  # MAB ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  # Obs
  mab0 <- read.table(paste0("../run/data/",country,"/out/mab_hfd.txt"), skip = 2, header = T,
                     stringsAsFactors = F)
  obs_mab <- mab0[mab0>=1960,]
  
  if(country %in% c("ES", "IR")){
  mabp <- read.table(paste0("../run/data/",country,"/out/mab_parity_hfd.txt"), skip = 2, header = T,
                     stringsAsFactors = F)
  ...
  plot(obs_mab[,1], obs_mab[,2],
       ...
       cex.main=1,cex.axis=1.6, cex.lab=1.6, type="p",
       lwd = 2, lty = 1, ylab = "Mean Age at Birth", xaxt = 'n', frame.plot = F)
  if(country != "FR"){
  lines(obs_mab1[,2], obs_mab1[,1])
  lines(obs_mab1[,2], obs_mab2[,1])
  }
  points(seq(1960, endY-1,1), mean_mab[(1960-iniY+1):(length(mean_mab)-20)], lty = 1, lwd = 2, col = "red")  
  for (i in 1:length(mab)){
    lines(seq(1960, endY-1,1), mab[[i]][(1960-iniY+1):(length(mean_mab)-20)], lty = 3, col = "red")
  }
  points(seq(1960, endY-1,1), mean_mab1[(1960-iniY+1):(length(mean_mab1)-20)], lty = 1, lwd = 2, col = "orange")  
  for (i in 1:length(mab1)){
    lines(seq(1960, endY-1,1), mab1[[i]][(1960-iniY+1):(length(mean_mab1)-20)], lty = 3, col = "orange")
  }
  points(seq(1960, endY-1,1), mean_mab2[(1960-iniY+1):(length(mean_mab2)-20)], lty = 1, lwd = 2, col = "violet")
  for (i in 1:length(mab2)){
    lines(seq(1960, endY-1,1), mab2[[i]][(1960-iniY+1):(length(mean_mab2)-20)], lty = 3, col = "violet")
  }
  axis(side = 1, at = seq(1955, y2+10, 5), label= seq(1955, y2 + 10, 5), cex.axis = 1.6)
  legend(1986,28, c("Observed", "Simulated"),
         lty = c(1,1),lwd=2, bty="n", cex=1.6, col = c("black","red"))
  ...
  plot(obs_mab[,1], obs_mab[,2],
       ...
       cex.main=1,cex.axis=1.6, cex.lab=1.6, type="p",
       lwd = 2, lty = 1, ylab = "Mean Age at Birth", xaxt = 'n', frame.plot = F)
  points(seq(iniY, endY-1,1), mean_mab[1:107], lty = 1, lwd = 2, col = "red")  
  for (i in 1:length(mab)){
    lines(seq(iniY,endY-1,1), mab[[i]][1:107], lty = 3, col = "red")
  }
  axis(side = 1, at = seq(iniY, endY, 5), label= seq(iniY, endY, 5), cex.axis = 1.6)
  legend(1986,28, c("Observed", "Simulated"),
         lty = c(1,1),lwd=2, bty="n", cex=1.6, col = c("black","red"))
  ...
  # MA UNION ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  # obs 
  #obs_mau <- read.csv(paste0("../run/data/",country,"/out/mean_age_marriage_period.csv"), header = T,
  #                   stringsAsFactors = F)
  # Sim
  mau <- list()
  ...
  # Plot
  lines(seq(iniY, endY-1, 1), mean_mau[1:107],lwd = 2, lty = 1)
  points(seq(iniY, endY-1,1), mean_mau[1:107], lty = 1, lwd = 2, col = "red") 
  lines(seq(iniY, endY-1, 1), mean_maul[1:107], col = "violet")
  lines(seq(iniY, endY-1, 1), mean_aux_mau[1:107])
  lines(seq(iniY, endY-1, 1), mean_aux_mau_cohort[1:107], col = "orange")
...
# Obs - NO DATA FOR SPAIN
  ...
  plot(obs_unp[,1], obs_unp[,2],
       ...
       cex.main=1,cex.axis=1.6, cex.lab=1.6, type="l",
       lwd = 2, lty = 1, ylab = "% Unplanned Births", xaxt = 'n', frame.plot = F)
  lines(seq(iniY, endY-1,1), mean_sim_unp[c((iniY-iniY+1):(length(mean_sim_unp)-20))], lty = 1, lwd = 2, col = "red")  
  for (i in 1:ncol(sim_unp)){
    lines(seq(iniY,endY-1,1), sim_unp[,i][c((iniY-iniY+1):(nrow(sim_unp)-20))], lty = 3, col = "red")
  }
  axis(side = 1, at = seq(iniY, endY-1+10, 10), label= seq(iniY, endY-1 + 10, 10), cex.axis = 1.6)
  ...
  plot(obs_unp[,1], obs_unp[,2],
       ...
       cex.main=1,cex.axis=1.6, cex.lab=1.6, type="l",
       lwd = 2, lty = 1, ylab = "% Unplanned Births", xaxt = 'n', frame.plot = F)
  lines(seq(iniY, endY-1,1), mean_sim_unp[1:107], lty = 1, lwd = 2, col = "red")  
  for (i in 1:ncol(sim_unp)){
    lines(seq(iniY,endY-1,1), sim_unp[,i][1:107], lty = 3, col = "red")
  }
  axis(side = 1, at = seq(iniY, endY-1+10, 5), label= seq(iniY, endY-1 + 10, 5), cex.axis = 1.6)
  legend(1986,.18, c("Observed FRANCE!", "Simulated"),
         lty = c(1,1),lwd=2, bty="n", cex=1.6, col = c("black","red"))
  ...
  plot(obs_unw[,1], obs_unw[,2],
       ...
       cex.main=1,cex.axis=1.6, cex.lab=1.6, type="l",
       lwd = 2, lty = 1, ylab = "% Unplanned Births", xaxt = 'n', frame.plot = F)
  lines(seq(1960, endY-1,1), mean_sim_unw[c((1960-iniY+1):(length(mean_sim_unw)-20))], lty = 1, lwd = 2, col = "red")  
  for (i in 1:ncol(sim_unw)){
    lines(seq(1960,endY-1,1), sim_unw[,i][c((1960-iniY+1):(nrow(sim_unw)-20))], lty = 3, col = "red")
  }
  axis(side = 1, at = seq(1955, endY-1+10, 5), label= seq(1955, endY-1 + 10, 5), cex.axis = 1.6)
  legend(1986,.18, c("Observed FRANCE!", "Simulated"),
         lty = c(1,1),lwd=2, bty="n", cex=1.6, col = c("black","red"))
  ...
  lines(seq(iniY, endY-1,1), mean_sim_unw[1:107], lty = 1, lwd = 2, col = "red")  
  for (i in 1:ncol(sim_unw)){
    lines(seq(iniY,endY-1,1), sim_unw[,i][1:107], lty = 3, col = "red")
  }
  axis(side = 1, at = seq(iniY, endY-1+10, 5), label= seq(iniY, endY-1 + 10, 5), cex.axis = 1.6)
  legend(1986,.18, c("Observed FRANCE!", "Simulated"),
         lty = c(1,1),lwd=2, bty="n", cex=1.6, col = c("black","red"))
  ...
  # DESIRED ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  if(country == "FR"){
    obsD <- read.table(paste0("../run/data/",country,"/out/ideal_sit.csv"), sep = ",", skip = 0, header = T)
    obsD <- obsD[!is.na(obsD[,5]),c(2,5)]
  }else{
    obsD <- read.table(paste0("../run/data/",country,"/out/ideal_sit.csv"), skip = 0, header = T)
  }
  ...
  mean_prop_w <- cbind(mean_prop_w[1:107,], seq(iniY,endY-1,1))
  #dKids1 <- smooth.spline(dKids,spar = 0.8)
  #dKids1 <- predict(dKids1)
  
  par(mfrow=c(1, 1))
  plot(seq(iniY,endY-1,1),mean_dKids[1:107], ylim=c(1,4),
       cex.main=1, cex.axis=1.6, 
       cex.lab=1.6, type="l", lty=1,
       lwd=2, cex.lab=1.6,
       ylab="Desired Family Size", xlab="Year", xaxt='n', frame.plot=F)
  for (i in 1:length(dKids)){
    lines(seq(iniY,endY-1,1), dKids[[i]][1:107], lty = 3, col = "red")
  }
  ...
  axis(side=1, at=seq(iniY, endY-1, 10), label= seq(iniY, endY-1, 10), cex.axis=1.6)
  legend(1990, 2.8,c("Simulated Ideal","Observed Ideal"),
         lty=c(1,NA),pch = c(NA, 1),lwd=2, bty="n", cex=1.6, col = c("black","black"))
  ...
  # Obs css
  css  <- read.table(paste0("../run/data/",country,"/out/childlessness.csv"), sep = ",", skip = 3, header = T)
...
plot(seq(iniY,endY-1,1), mean_sim_gap[1:107],
     xlab="Year", ylim=c(-0.5,1),
     cex.main=1, cex.axis=1, 
     cex.lab=1,type="l", col="red",
     lwd=2, lty=1,ylab="Gap")
lines(seq(iniY,endY-1,1), mean_sim_gap3[1:107],col="Black", lwd = 2)
